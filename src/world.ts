...
type TypeStorage<T> = Map<string, T>;
type ComponentStorage<T> = Map<Entity, T>;
...
export class World {
  ...
  private components: TypeStorage<ComponentStorage<Component>> = new Map();
  ...
  destroy(entity: Entity) {
    ...
    for (const value of this.components) {
      const storage = value[1];
      const component = storage.get(entity);
      if (storage.delete(entity) && component?.free !== undefined) {
        component.free();
      }
    }
  }
  ...
  has<T extends Component>(entity: Entity, component: Constructor<T>): boolean {
    const type = component.name;
    const storage = this.components.get(type);
    return storage !== undefined && storage.get(entity) !== undefined;
  }
  ...
  remove<T extends Component>(
    ...
    const storage = this.components.get(type);
    if (storage === undefined) return undefined;
    const out = storage.get(entity) as T | undefined;
  ...
  view<T extends Constructor<Component>[]>(...types: T): View<T> {
    ...
    if (view === undefined) {
      ...
      for (let i = 0; i < types.length; ++i) {
        if (this.components.get(types[i].name) === undefined) {
          this.components.set(types[i].name, new Map<Entity, T>());
        }
      }
      view = new ViewImpl(this, types);
...
function generateView(world: World, types: any[]): ComponentView<any> {
  ...
  for (let i = 0; i < length; ++i) {
    ...
    storages += `const ${name} = ${keywords.world}.components.get("${typeName}");\n`;
  ...
  for (let i = 0; i < length; ++i) {
    ...
    variables += `const ${name} = ${storageNames[i]}.get(${keywords.entity});\n`;
